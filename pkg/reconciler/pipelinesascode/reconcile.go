...
// converge the two.
func (r *Reconciler) ReconcileKind(ctx context.Context, pac *v1alpha1.PipelinesAsCode) pkgreconciler.Event {
	logger := logging.FromContext(ctx).With("name", pac.GetName())
	pac.Status.InitializeConditions()
	pac.Status.SetVersion(r.pacVersion)

	if pac.GetName() != v1alpha1.PipelinesAsCodeName {
		msg := fmt.Sprintf("Resource ignored, Expected Name: %s, Got Name: %s",
			v1alpha1.PipelinesAsCodeName,
			pac.GetName(),
		)
		logger.Error(msg)
	...
	if _, err := common.PipelineReady(r.pipelineInformer); err != nil {
		if err.Error() == common.PipelineNotReady || err == tektonoperatorv1alpha1.DEPENDENCY_UPGRADE_PENDING_ERR {
	...
	if err := r.installerSetClient.MainSet(ctx, pac, &r.manifest, filterAndTransform(r.extension)); err != nil {
		...
		logger.Error(msg)
		if err == tektonoperatorv1alpha1.REQUEUE_EVENT_AFTER {
			return err
		}
