...
// controller reconciling v1alpha1.PipelinesAsCode.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.PipelinesAsCode. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.PipelinesAsCode) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.PipelinesAsCode.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.PipelinesAsCode. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.PipelinesAsCode) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.PipelinesAsCode if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.PipelinesAsCode.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.PipelinesAsCode) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.PipelinesAsCode) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.PipelinesAsCode resources.
type reconcilerImpl struct {
	...
	Lister operatorv1alpha1.PipelinesAsCodeLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister operatorv1alpha1.PipelinesAsCodeLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.PipelinesAsCode, desired *v1alpha1.PipelinesAsCode) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.OperatorV1alpha1().PipelinesAsCodes()
		...
		updater := r.Client.OperatorV1alpha1().PipelinesAsCodes()
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.PipelinesAsCode, desiredFinalizers sets.String) (*v1alpha1.PipelinesAsCode, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.OperatorV1alpha1().PipelinesAsCodes()
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.PipelinesAsCode) (*v1alpha1.PipelinesAsCode, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.PipelinesAsCode, reconcileEvent reconciler.Event) (*v1alpha1.PipelinesAsCode, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
